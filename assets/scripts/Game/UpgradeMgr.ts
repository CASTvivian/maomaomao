...
export default class UpgradeMgr extends cc.Component {
    ...
    // 升级经验
    private _jingyanArr: number[] = [5, 10, 20, 25, 30, 40, 50, 60, 60, 70, 80, 90, 100, 110, 120, 130, 150, 170, 200, 220, 250, 280, 300, 350, 400, 500, 1000];// 经验数组
            ...
            case Constant.E_Zombie_Death: {
                if (event.node) {
                    if (this.isHaveSkill(SkillType.子弹碎片) && event.from) {
                        ...
                        if (gameMgr && gameMgr.playerTs && gameMgr.playerTs.HP < gameMgr.playerTs.totleHp && Math.random() < 0.05) {
                            gameMgr.playerTs.HP += 1;
